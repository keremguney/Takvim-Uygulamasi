...
from PyQt5.QtWidgets import QApplication, QMainWindow, QLabel, QPushButton, QCalendarWidget, QVBoxLayout, QWidget, QLineEdit, QMessageBox, QComboBox, QDialog, QTimeEdit
...
class CalendarWindow(QMainWindow):
    def __init__(self, user_id, db_connection):
        ...
        self.calendar_widget.setGeometry(20, 20, 750, 550)
        ...
        self.timer.timeout.connect(self.check_event_time)
    ...
    def open_event_dialog(self, date):
        ...
        try:
            ...
            if event is not None:
                self.edit_event_dialog = EditEventDialog(self, event[2], event[3], event[4])
    ...
    def add_event(self, date, time, event, event_type):
        insert_event_query = """
            INSERT INTO events (date, event, time, event_type, user_id) VALUES (?, ?, ?, ?, ?)
        """
        try:
            cursor = self.db_connection.cursor()
            cursor.execute(insert_event_query, (date, event, time, event_type, self.user_id))
            self.db_connection.commit()
            QMessageBox.information(self, "Etkinlik Eklendi", "Etkinlik başarıyla eklendi!")
        except sqlite3.Error as e:
            print(f"SQLite veritabanı hatası: {e}")

    def update_event(self, date, time, event, event_type):
        update_event_query = """
            UPDATE events SET event=?, event_type=?, time=? WHERE date=? AND user_id=?
        """
        try:
            cursor = self.db_connection.cursor()
            cursor.execute(update_event_query, (event, event_type, time, date, self.user_id))
            self.db_connection.commit()
    ...
    def check_event_time(self):
        ...
        select_event_query = """
            SELECT * FROM events WHERE time=? AND user_id=?
        """
        try:
            cursor = self.db_connection.cursor()
            cursor.execute(select_event_query, (current_time.toString("HH:mm"), self.user_id))
            event = cursor.fetchone()

            if event is not None:
                QMessageBox.information(self, ("Uyari", "Etkinlik zamani"))
        except sqlite3.Error as e:
            print(f"SQLite veritabanı hatası: {e}")  
...
class AddEventDialog(QDialog):
    def __init__(self, parent, selected_date):
        ...
        self.event_input.move(150, 50)

        self.time_label = QLabel("Saat:", self)
        self.time_label.move(50, 100)

        self.time_edit = QTimeEdit(self)
        self.time_edit.move(150, 100)

        self.event_type_label = QLabel("Etkinlik Türü:", self)
        self.event_type_label.move(50, 150)
        ...
        self.event_type_combobox.move(150, 150)

        self.add_event_button = QPushButton("Ekle", self)
        self.add_event_button.move(100, 200)
    ...
    def add_event(self):
        ...
        self.parent().add_event(self.selected_date, time, event, event_type)
        self.accept()


class EditEventDialog(QDialog):
    def __init__(self, parent, time, event, event_type):
        super().__init__(parent)
        ...
        self.update_event_button.move(100, 200)
    ...
    def update_event(self):
        ...
        self.parent().update_event(self.parent().calendar_widget.selectedDate().toString("yyyy-MM-dd"), time, event, event_type)
        self.accept()
